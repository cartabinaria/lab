...
\begin{frame}{Intallare per la prima volta Linux}
  ...
  \begin{enumerate}
    \item <1-> Installarlo in \textbf{dual-boot}: Se avete un pc con installato
      ...
      \only<1> {\bigskip ATTENZIONE: non sto parlando della WSL (Windows Subsystem 
...
\begin{frame}{Wildcard}
  ...
  Si possono combinare anche più asterischi: \texttt{c*a*} fa metch con tutte le
...
\begin{frame}{Read, write, execute}
  Ogni file ha un strina formata da \textbf{9 bit} che determina quelli permessi 
  specifici ha quel file rispetto all'\textit{utente}, il \textit{gruppo} e gli 
  \textit{altri}.\bigskip
