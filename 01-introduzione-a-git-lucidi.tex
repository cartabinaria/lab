...
\begin{frame}
  ...
  \only<2>{
    ...
    \caption{Log dei commit dopo un merge}
  }
  \end{figure}
...
\begin{frame}
  ...
  aggiunto nessun commit, si dice viene fatto un \emph{fast forward}}:
  \begin{figure}
    \centering
...
\begin{frame}
  ...
  \begin{block}{Consiglio}
    ...
    \emph{per branch} a cui inviare i cambiamenti. La flag \texttt{-u} del
    comando \texttt{push} imposta un remoto come \texttt{upstream}.
  \end{block}
\end{frame}
